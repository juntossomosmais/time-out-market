...
async function mockServiceFunction(..._args: unknown[]) {
...
const mockProvider: Provider = {
  ...
  setItem: (key, value) => {
    // @ts-expect-error - Testing with mock provider type
    provider[key] = value
  },
  removeItem: (key) => {
    delete provider[key]
...
function cachedServiceFunction<TParams extends unknown[], TResult>() {
  const cachedFunction = async (...params: TParams): Promise<TResult> => {
    return cacheFactory<TParams, TResult>({
      params,
      expire: expireTime,
      // @ts-expect-error - Testing with mock service function
...
describe('cacheFactory', () => {
  ...
  test.each([
    ...
    async (params, hashedParams) => {
      ...
      const expiredCompressedEntry = deflate(JSON.stringify(entry))
  ...
  function anotherCachedServiceFunction<TParams extends unknown[], TResult>() {
    const cachedFunction = async (...params: TParams): Promise<TResult> => {
      return cacheFactory<TParams, TResult>({
        params,
        expire: expireTime,
        // @ts-expect-error - Testing with another mock function
